...
Αντί να κρατάει όλα τα δεδομένα ενός tenant σε ένα σημείο, το Entra ID σπάει τα δεδομένα σε ανεξάρτητες μονάδες κλιμάκωσης, τα λεγόμενα **partitions**. Κάθε partition έχει τη δική του δομή για writes και reads, κάτι που επιτρέπει στη Microsoft να κλιμακώνει το σύστημα σε παγκόσμιο επίπεδο χωρίς ένα partition να «σέρνει» ολόκληρο το σύστημα μαζί του.

### Primary replica vs. secondary replicas: η καρδιά της αρχιτεκτονικής

Αυτό κατά τη γνώμη μου είναι το πιο σημαντικό σημείο για να καταλάβεις πώς «σκέφτεται» το Entra ID:
